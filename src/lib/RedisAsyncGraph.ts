...
import { RedisAsyncMap } from "./RedisAsyncMap"
...
export class RedisAsyncGraph extends AbstractAsyncGraph<Node, Edge> {
  private id = UUID.v4()
  private edges: RedisAsyncMap<Node[]> = new RedisAsyncMap("list", `edges.${this.id}`)
  private edgeWeights: RedisAsyncMap<number> = new RedisAsyncMap("key", `edgeWeights.${this.id}`)
  ...
  protected async addNodeInternal(node: Node) {
    await this.edges.set(node, await this.adjacentInternal(node))
    return node
  }

  protected async removeNodeInternal(node: Node) {
    const nodes = await this.edges.keys()
    for (const u of nodes) {
      for (const v of await this.edges.get(u) as unknown as Node) {
        if (v === node) {
    ...
    await this.edges.delete(node)
  }

  protected async nodesInternal() {
    const nodeSet = new Set<string>()
    for (const node of await this.edges.keys()) {
      nodeSet.add(node)
      for (const edge of await this.edges.get(node) as unknown as Node) {
        nodeSet.add(edge)
      }
    }
  ...
  protected async adjacentInternal(node: Node) {
    if (!await this.edges.has(node)) {
      return []
    }
    return await this.edges.get(node) as unknown as Promise<Node[]>
  }
  ...
  protected async getEdgeWeightInternal(u: string, v: string) {
    const edge = this.encodeEdge(u, v)
    const weight = await this.edgeWeights.get(edge)
    return weight === undefined || weight === null ? 1 : weight
  }
  ...
  protected async indegreeInternal(node: Node) {
    ...
    for (const key of await this.edges.keys()) {
      for (const link of await this.edges.get(key) as unknown as Node) {
        if (link === node) {
          degree++
        }
      }
  ...
  protected async outdegreeInternal(node: Node) {
    if (await this.edges.has(node)) {
      const edge = await this.edges.get(node) as unknown as Node
      return edge.length
    }
    return 0
  }
